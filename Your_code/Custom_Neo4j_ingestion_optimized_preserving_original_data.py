...
class Neo4jIngestionComponent(Component):
    ...
    description = "Load data into Neo4j while preserving existing data"
    icon = "git-graph"
    name = "NEO4J"
    ...
    def create_or_merge_document_nodes(self, documents):
        """
        Create or merge document nodes in the graph.
        
        :param documents: Dictionary of document information
        :return: Number of created or merged nodes
        """
        self.logger.info("Creating or merging document nodes")
        merge_document_node = """
        ...
        self.logger.info(f"Created or merged {node_count} document nodes")
        return node_count

    def create_or_merge_chunk_nodes(self, chunks):
        """
        Create or merge chunk nodes in the graph.
        
        :param chunks: List of chunk dictionaries
        :return: Number of created or merged nodes
        """
        self.logger.info("Creating or merging chunk nodes")
        merge_chunk_node = """
        ...
        node_count = 0
        for chunk in chunks:
            self.logger.debug(f"Creating or merging `:Chunk` node for chunk ID {chunk['chunkId']}")
            self.kg.query(merge_chunk_node, params={'params': chunk})
            node_count += 1
            
        self.logger.info(f"Created or merged {node_count} chunk nodes")
        return node_count

    def create_constraints_and_indexes(self):
        """Create necessary constraints and indexes in the graph if they don't exist."""
        self.logger.info("Creating constraints and indexes")
        self.create_constraint_if_not_exists(
            "CREATE CONSTRAINT unique_chunk IF NOT EXISTS FOR (c:Chunk) REQUIRE c.chunkId IS UNIQUE",
            "Chunk.chunkId uniqueness constraint"
        )
        
        self.create_constraint_if_not_exists(
            "CREATE CONSTRAINT unique_document IF NOT EXISTS FOR (d:Document) REQUIRE d.fileName IS UNIQUE",
            "Document.fileName uniqueness constraint"
        )
    ...
    def create_embeddings(self):
        ...
        self.logger.info("Creating embeddings for new chunks")
    ...
    def create_constraint_if_not_exists(self, query, constraint_description):
        """
        Create a constraint if it doesn't already exist.
        
        :param query: The query to create the constraint
        :param constraint_description: A description of the constraint for logging
        """
        try:
            self.kg.query(query)
            self.logger.info(f"Created constraint: {constraint_description}")
        except Exception as e:
            if "ConstraintAlreadyExists" in str(e):
                self.logger.info(f"Constraint already exists: {constraint_description}. Skipping creation.")
            else:
                self.logger.error(f"Error creating constraint {constraint_description}: {str(e)}")
    ...
    def build_output(self) -> Data:
        ...
        document_count = self.create_or_merge_document_nodes(documents)
        chunk_count = self.create_or_merge_chunk_nodes(chunks)
        ...
        return Data(text=f"Processed {total_nodes} nodes ({document_count} documents, {chunk_count} chunks)")